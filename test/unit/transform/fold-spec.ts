...
describe('View.transform(): fold', () => {
  ...
  it('default', () => {
    ...
    expect(keys(dv.rows[0])).to.eql(['key', 'value']);
  });
  ...
  it('key and value', () => {
    ...
    expect(dv.rows.length).to.equal(8);
    expect(keys(dv.rows[0])).to.eql(['type', 'typeValue']);
  });
